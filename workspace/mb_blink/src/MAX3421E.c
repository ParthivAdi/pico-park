...
void MAXreg_wr(BYTE reg, BYTE val) {
    ...
    //write reg + 2 via SPI
    BYTE send_buffer[2] = {reg + 2, val};
    //write val via SPI
    Status = XSpi_Transfer(&SpiInstance, send_buffer, NULL, 2);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	for(int x =0; x < nbytes; x++)
		send_buffer[x+1] = data[x];
	Status = XSpi_Transfer(&SpiInstance, send_buffer, NULL, nbytes + 1);
	if (Status != 0)
...
BYTE MAXreg_rd(BYTE reg) {
    ...
    XSpi_SetSlaveSelect(&SpiInstance, 1);
    BYTE send_buffer[2] = {reg, 0};
    BYTE recv_buffer[2] = {0};
